...
if %errorlevel% neq 0 (
    echo Python not found. Downloading and installing Python 3.11... | tee -a %LOGFILE%
    ...
    if exist "%TEMP%\python_installer.exe" (
        ...
        echo Failed to download Python installer. | tee -a %LOGFILE%
        exit /b 1
    )
)
...
if %errorlevel% neq 0 (
    echo Python version is not 3.11+. Please upgrade Python. | tee -a %LOGFILE%
    exit /b 1
)
...
if %errorlevel% neq 0 (
    echo Failed to install Python dependencies. | tee -a %LOGFILE%
    exit /b 1
)
